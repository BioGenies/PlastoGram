...
predict_profileHMM <- function(test_seqs, hmmer_dir = Sys.which("hmmsearch")) {
  ...
  if(hmmer_dir == "") {
    ...
    system(paste0("hmmsearch --tblout hmmer_res_sec ", normalizePath(system.file(package = "PlastoGram")), "/PlastoGram_Sec_model.hmm test_seqs.fa")) 
    system(paste0("hmmsearch --tblout hmmer_res_tat ", normalizePath(system.file(package = "PlastoGram")), "/PlastoGram_Tat_model.hmm test_seqs.fa")) 
  } else {
    ...
    tryCatch(system(paste0(hmmer_dir, "/hmmsearch --tblout hmmer_res_sec ", normalizePath(system.file(package = "PlastoGram")), 
                           ...
                           normalizePath(system.file(package = "PlastoGram")), "/PlastoGram_Tat_model.hmm test_seqs.fa")),
             warning = function(w) {
